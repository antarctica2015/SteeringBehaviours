...
//////////////////////////////////////////////////////////////////////////
//
//  Some useful template functions (from book)
//  Would like to remove dependancy on them
//
//////////////////////////////////////////////////////////////////////////
...
bool Overlapped(const T* ob, const conT& conOb, double MinDistBetweenObstacles = 40.0)
{
  ...
  {
    //need to change this to check for sphere overlap really?
...
void TagNeighbors(const T& entity, conT& ContainerOfEntities, double radius)
{
       ...
       ++curEntity)
  {
	...
	//should zero out y component?
	Vector3 to = (*curEntity)->GetPosition() - entity->GetPosition();
    ...
    if ( ((*curEntity) != entity) && ((double)to.squaredLength() < range*range))
    {
      (*curEntity)->Tag();
    }
                                    ...
                                    const conT& ContainerOfEntities)
{
       ...
       ++curEntity)
  {
    ...
    Vector3 ToEntity = entity->GetPosition() - (*curEntity)->GetPosition();

    double DistFromEachOther = (double)ToEntity.length();
    ...
    if (AmountOfOverLap >= 0)
    {
      ...
      entity->SetPosition(entity->GetPosition() + (ToEntity/DistFromEachOther) * AmountOfOverLap);
    }
  }//next entity
}
